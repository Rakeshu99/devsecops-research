...
| Microsoft Sentinel | Root cause identified for 0 incidents/alerts; remediation in progress | Workspace and trial active (10 Jul–8 Aug 2026), zero ingestion/cost confirmed via KQL; diagnosed that tenant Cloud Security onboarding (unified Defender portal) had never been completed, separate from both the Defender for Cloud connector and the Sentinel connector gallery; tenant preparation initiated 10 Jul 2026, 09:22 PM, ETA ~6 hrs, re-verification pending — see `docs/implementation-log.md`, 10 July entry |
| Azure Policy | Assigned — evaluation pending | Pre-existing "ASC Default" initiative found auto-assigned by Defender for Cloud (226 policies, zero manual config); additionally assigned "Microsoft cloud security benchmark v2" (730 policies, 92 groups) at subscription scope, Audit effect, confirmed 10 Jul 2026, 10:02 PM; compliance results pending — see `docs/implementation-log.md`, 10 July entry |
...
**Notable finding — Sentinel/Defender for Cloud integration is not automatic:** despite Defender for Cloud generating 75 confirmed findings, zero incidents or alerts ever appeared in Sentinel or the unified Defender portal. Root-cause diagnosis (Incidents → Alerts → Data Connectors → Cloud Security Overview) found that the tenant had never been "prepared" for the unified portal's Cloud Security experience — a distinct, one-time onboarding step not surfaced by either the Defender for Cloud connector setup or the Sentinel data connector gallery. A further discrepancy was observed between the Defender portal's Data Connectors view (showing 1 connector) and the native Azure Sentinel blade's Data Connectors view for the same workspace (showing 7 connectors, all unrelated M365 Defender XDR family connectors) — evidence of ongoing platform-transition inconsistency between the two UIs Microsoft is mid-way through unifying. This is a citable Metric 3 (Setup Complexity) finding: two products that appear integrated in the interface require a non-obvious, separately-triggered dependency to actually connect.

**Notable finding — Azure Policy ships partial governance by default:** a "ASC Default" initiative (226 policies) was found already assigned to the subscription automatically by Defender for Cloud, with no manual configuration. Achieving comprehensive coverage required deliberately assigning the full "Microsoft cloud security benchmark v2" initiative (730 policies) on top of this. This is a relevant SME suitability finding: Azure's default security posture is real but partial, and teams assuming "secure by default" without checking initiative coverage would be operating on the narrower 226-policy baseline rather than the comprehensive 730-policy standard.
...
| Azure — Microsoft Sentinel | 🔶 In progress — workspace and trial active (10 Jul–8 Aug 2026), zero ingestion/cost confirmed; root cause identified for zero incidents/alerts (tenant Cloud Security onboarding gap); tenant preparation initiated 10 Jul 2026 09:22 PM, ETA ~6 hrs, re-verification pending |
| Azure — Azure Policy | 🔶 In progress — pre-existing ASC Default initiative (226 policies) documented; Microsoft cloud security benchmark v2 (730 policies) assigned at subscription scope 10 Jul 2026 10:02 PM, compliance evaluation pending |
...
**Next step:** Re-verify Sentinel Incidents/Alerts population and Azure Policy compliance results (both pending as of 10 Jul 2026, ~10 PM), close out both as Complete in this README once confirmed, then build the Azure stack GitHub Actions pipeline for equivalent timing comparison. Note: pipeline timing figures for the open-source stack (baseline ~12s, stack ~27s) were captured before the CI submodule fix and require re-measurement under corrected conditions before use in the comparative analysis.
